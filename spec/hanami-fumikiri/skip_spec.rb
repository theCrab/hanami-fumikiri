describe Hanami::Fumikiri do
  ...
  describe 'passes with faulty token' do
    it 'raises error when invalid token' do
      expect { action.new.call('Authentication' => 'not_so_valid_token') }
        .not_to raise_error
    end
    ...
    describe 'missing payload data keys' do
      before do
        @valid_data = {
          sub: user.id,                 # subject:
          iat: Time.now.to_i,           # issued_at: DateTime when it created
      ...
      it 'no sub' do
        ...
        expect { action.new.call('Authentication' => 'Bearer ' + invalid_token) }
          .not_to raise_error
      end
